...
function drawWheel() {
    ...
    for (let i = 0; i < alphabet.length; i++) {
        ...
        ctx.fillText(alphabet[i], radius - 10, 10);
        ctx.restore();
    }
}
...
function spin() {
    ...
    isSpinning = true;
    result.textContent = '';
    const spinAngle = Math.floor(Math.random() * 360) + 1440; // At least 4 full rotations
    ...
    function animate(time) {
        ...
        if (progress < 1) {
            ...
            const selectedIndex = Math.floor(((finalAngle + 360 / (2 * alphabet.length)) % 360) / (360 / alphabet.length));
